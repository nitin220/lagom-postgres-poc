...
class HelloEntity extends PersistentEntity {
  ...
  /**
    ...
    */
  override def initialState: HelloState = HelloState(Hello(0L,"Hello"), LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    ...
    }/*.onReadOnlyCommand[GetGreetingMessage, String] {

      // Command handler for the Hello command
      case (GetGreetingMessage(id), ctx, state) =>
        // Reply with a message built from the current message, and the name of
        // the person we're meant to say hello to.
        ctx.reply(s"Hello $id!")
...
/**
  ...
  */
case class HelloState(message: Hello, timestamp: String)
...
/**
  ...
  */
case class GreetingMessageChanged(message: Hello) extends HelloEvent
...
/**
  ...
  */
case class UseGreetingMessage(message: Hello) extends HelloCommand[Done]

case class GetGreetingMessage(message: Hello) extends HelloCommand[Done]
